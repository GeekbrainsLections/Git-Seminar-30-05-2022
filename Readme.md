...
* Для того чтобы получить удаленный репозиторий нужно воспользоваться командой *git clone* и ввести ссылку на репозиторий на Git hub(ее можно взять с сайта Git Hub c вкладки Code)

Пример *git clone https://github.com/SergeiEremkin/GB03.git*

* Так же можно сказачть репозиторий ZIP-архивом. Обычно это медленее и мы не сможем отслеживать изменения. Теряется смысл Git Hub. Ориентируйтесь по ситуации.

## Скачивание изменений с удалённого репозитория

Для того чтобы получить изменния с удаоенного репозитория, необходимо воспользоваться командой *git pull*. В терминале вводим *git pull* и вытягиваем изменения в наш локальный репозиторий. Если хотим получить изменния с конкретной ветки то можно воспользоваться командой *git pull origin название ветки*

## Отправка изменений в удалённый репозиторий

После того, как мы доработали проект и хотим отправить изменения на удаленный репозиторий нам понадобиться команда *git push*. В терминале вводим *git push*. Так же, если хотим внести изменния в конкретную ветку то вводим команду *git push origin название ветки*.
