...
Для получения удаленного репозитория используется команда *git clone*. Для этого нужно скопировать **ссылку** удаленного репозитория на **Github**. Далее нужно прописать команду в терминале *git clone <ссылка удаленного репозитория>*.
